...
import { EncodedContractFunction, SignedTxExecutionRequest, Tx, TxHash, L2Logs } from '@aztec/types';
...
export function fromTxMessage(buffer: Buffer): Tx {
  ...
  const encryptedLogs = toObject(txRequest.remainingData, L2Logs);
  if (!encryptedLogs.obj) {
    encryptedLogs.obj = new L2Logs([]);
  }
